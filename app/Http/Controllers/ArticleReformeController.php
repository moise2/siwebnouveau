...
class ArticleReformeController extends Controller
{
    // Afficher la liste des articles de la catégorie "réforme" avec pagination
    public function index()
{
    $title = 'Actualité sur les réformes';
    
    // Récupérer les articles de la catégorie "Réformes"
    $articles = Post::whereHas('categories', function ($query) {
        $query->where('name', 'Réformes');
    })
    ->latest('published_at') // <--- AJOUT DE CETTE LIGNE
    ->paginate(45); // 21 articles par page (vous avez mis 44, j'ai gardé 44)

    $hasCategory = $articles->isNotEmpty();

    $autoSelectedCategories = Category::where('name', 'Réformes')->pluck('id')->toArray();
    ...
    public function show($slug)
    {
        ...
        $relatedArticles = Post::with('categories')
            ->whereHas('categories', function ($query) {
                ...
                $query->where('categories.name', 'Réformes');
