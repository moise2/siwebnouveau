...
class DocAmiController extends Controller
{
    ...
    {
        $title = 'Documents AMI (Appel à Manifestation d\'Intérêt)';
        $categoryName = 'AMI'; // Catégorie spécifique pour ce contrôleur
        ...
        // Récupérer tous les documents appartenant à la catégorie 'AMI'
        ...
        $documents = Document::with('categories')
            ...
            ->paginate(44); // Pagination par 44 comme dans DocAllDetteController

        // Toutes les catégories avec des documents pour le filtre dropdown
        // (Comme dans DocAllDetteController, cela inclura toutes les catégories qui ont des documents)
        $allCategories = Category::whereHas('documents')->orderBy('name')->get();

        // Les catégories auto-sélectionnées (AMI dans ce cas) pour la vue
        ...
        $bladeRouteNames = [
            'show' => 'documents.ami.show',
            'download' => 'documents.ami.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // Pour le JavaScript
                'search' => route('documents.ami.search'),
                'show' => route('documents.ami.show', ['slug' => 'PLACEHOLDER_SLUG']),
                'download' => route('documents.ami.download', ['id' => 'PLACEHOLDER_ID'])
            ],
    ...
    {
        Log::debug('Search request received for AMI documents', [
        ...
        $categoryName = "AMI"; // Catégorie spécifique pour la recherche
        $targetCategoryIds = Category::where('name', $categoryName)->pluck('id')->toArray();

        // Construction de la requête de base, filtrée par la catégorie 'AMI'
        ...
        // Note: Le filtre 'categories' est inclus ici, mais les catégories pré-sélectionnées ('AMI')
        // sont déjà appliquées par $targetCategoryIds. Si l'utilisateur sélectionne d'autres
        // catégories, cela combinera les filtres.
        ...
        // Note: Celles-ci s'ajouteront au filtre 'AMI' déjà en place
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("Le chemin du fichier dans la base de données n'est pas une chaîne pour le document (méthode show)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("Fichier non trouvé sur le disque pour affichage (méthode show)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("Le chemin du fichier dans la base de données n'est pas une chaîne pour le document (méthode download)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("Fichier introuvable sur le disque pour téléchargement (méthode download)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
