...
use Illuminate\Http\JsonResponse; // Added for JsonResponse return type
use Symfony\Component\HttpFoundation\BinaryFileResponse; // Added for BinaryFileResponse return type
use Symfony\Component\HttpFoundation\Response; // Added for Response return type
use Carbon\Carbon; // Added for date handling if needed

class DocBudgetCitoyenController extends Controller
{
    ...
    {
        $title = 'Documents de Budget Citoyen';
        $categoryName = 'budget citoyen'; // Specific category for this controller
        ...
        // Retrieve all documents linked to the 'budget citoyen' category
        ...
        // Auto-selected categories ('budget citoyen' in this case) for the view
        ...
        $bladeRouteNames = [
            'show' => 'documents.budget_citoyen.show',
            'download' => 'documents.budget_citoyen.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // For JavaScript
                'search' => route('documents.budget_citoyen.search'),
                'show' => route('documents.budget_citoyen.show', ['slug' => 'PLACEHOLDER_SLUG']),
                'download' => route('documents.budget_citoyen.download', ['id' => 'PLACEHOLDER_ID'])
            ],
    ...
    {
        Log::debug('Search request received for "Budget Citoyen" documents', [
        ...
        $categoryName = "budget citoyen"; // Specific category for search
        $targetCategoryIds = Category::where('name', $categoryName)->pluck('id')->toArray();

        // Build the base query, filtered by the 'budget citoyen' category
        ...
        // Note: These will be added to the 'budget citoyen' filter already in place
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (show method - Budget Citoyen)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for display (show method - Budget Citoyen)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (download method - Budget Citoyen)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for download (download method - Budget Citoyen)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
