...
class DocBudgetDepensesController extends Controller
{
    ...
    {
        $title = 'Documents Budget Dépenses';
        $categoryName = 'budget dépense'; // Specific category for this controller
        ...
        // Retrieve all documents linked to the 'budget dépense' category
        ...
        // Auto-selected categories ('budget dépense' in this case) for the view
        ...
        $bladeRouteNames = [
            'show' => 'documents.budget_depense.show',
            'download' => 'documents.budget_depense.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // For JavaScript
                'search' => route('documents.budget_depense.search'),
                'show' => route('documents.budget_depense.show', ['slug' => 'PLACEHOLDER_SLUG']),
                'download' => route('documents.budget_depense.download', ['id' => 'PLACEHOLDER_ID'])
            ],
    ...
    {
        Log::debug('Search request received for "Budget Dépenses" documents', [
        ...
        $categoryName = "budget dépense"; // Specific category for search
        $targetCategoryIds = Category::where('name', $categoryName)->pluck('id')->toArray();

        // Build the base query, filtered by the 'budget dépense' category
        ...
        // Note: These will be added to the 'budget dépense' filter already in place
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (show method - Budget Dépenses)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for display (show method - Budget Dépenses)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (download method - Budget Dépenses)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for download (download method - Budget Dépenses)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
