...
class DocBudgetEtatController extends Controller
{
    ...
    {
        $title = 'Documents Budget de l\'État';
        $categoryName = 'Budget de l\'Etat'; // Specific category for this controller
        ...
        // Retrieve all documents linked to the 'Budget de l\'Etat' category
        ...
        // Auto-selected categories ('Budget de l\'Etat' in this case) for the view
        ...
        $bladeRouteNames = [
            'show' => 'documents.budget_etat.show',
            'download' => 'documents.budget_etat.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // For JavaScript
                'search' => route('documents.budget_etat.search'),
                'show' => route('documents.budget_etat.show', ['slug' => 'PLACEHOLDER_SLUG']),
                'download' => route('documents.budget_etat.download', ['id' => 'PLACEHOLDER_ID'])
            ],
     ...
     * likely for showing all documents related to "Budget de l'Etat" without
    ...
    // {
    //     $title = 'Tous les Documents Budget de l\'État';
    //     $categoryName = 'Budget de l\'Etat';
    ...
    {
        Log::debug('Search request received for "Budget de l\'État" documents', [
        ...
        $categoryName = "Budget de l'Etat"; // Specific category for search
        $targetCategoryIds = Category::where('name', $categoryName)->pluck('id')->toArray();

        // Build the base query, filtered by the 'Budget de l\'Etat' category
        ...
        // Note: These will be added to the 'Budget de l\'Etat' filter already in place
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (show method - Budget de l'État)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for display (show method - Budget de l'État)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (download method - Budget de l'État)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for download (download method - Budget de l'État)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
