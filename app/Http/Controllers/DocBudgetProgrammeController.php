...
class DocBudgetProgrammeController extends Controller
{
    ...
    {
        $title = 'Documents Budgets Programme';
        $categoryName = 'Budget Programme'; // Specific category for this controller
        ...
        // Retrieve all documents linked to the 'Budget Programme' category
        ...
        // Auto-selected categories ('Budget Programme' in this case) for the view
        ...
        $bladeRouteNames = [
            'show' => 'documents.budget_programme.show',
            'download' => 'documents.budget_programme.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // For JavaScript
                'search' => route('documents.budget_programme.search'),
                'show' => route('documents.budget_programme.show', ['slug' => 'PLACEHOLDER_SLUG']),
                'download' => route('documents.budget_programme.download', ['id' => 'PLACEHOLDER_ID'])
            ],
    ...
    {
        Log::debug('Search request received for "Budget Programme" documents', [
        ...
        $categoryName = "Budget Programme"; // Specific category for search
        $targetCategoryIds = Category::where('name', $categoryName)->pluck('id')->toArray();

        // Build the base query, filtered by the 'Budget Programme' category
        ...
        // Note: These will be added to the 'Budget Programme' filter already in place
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (show method - Budget Programme)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for display (show method - Budget Programme)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (download method - Budget Programme)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for download (download method - Budget Programme)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
