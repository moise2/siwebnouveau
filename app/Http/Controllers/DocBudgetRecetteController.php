...
class DocBudgetRecetteController extends Controller
{
    ...
    {
        $title = 'Documents Budget Recette';
        $categoryName = 'Budget recette'; // Specific category for this controller
        ...
        // Retrieve all documents linked to the 'Budget recette' category
        ...
        // Auto-selected categories ('Budget recette' in this case) for the view
        ...
        $bladeRouteNames = [
            'show' => 'documents.recettes.show',
            'download' => 'documents.recettes.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // For JavaScript
                'search' => route('documents.recettes.search'),
                'show' => route('documents.recettes.show', ['slug' => 'PLACEHOLDER_SLUG']),
                'download' => route('documents.recettes.download', ['id' => 'PLACEHOLDER_ID'])
            ],
    ...
    {
        Log::debug('Search request received for "Budget recette" documents', [
        ...
        $categoryName = "Budget recette"; // Specific category for search
        $targetCategoryIds = Category::where('name', $categoryName)->pluck('id')->toArray();

        // Build the base query, filtered by the 'Budget recette' category
        ...
        // Note: These will be added to the 'Budget recette' filter already in place
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (show method - Budget Recette)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for display (show method - Budget Recette)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (download method - Budget Recette)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for download (download method - Budget Recette)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
