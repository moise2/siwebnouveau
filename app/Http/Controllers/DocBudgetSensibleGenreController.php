...
class DocBudgetSensibleGenreController extends Controller
{
    ...
    {
        $title = 'Documents Budgétaires Sensibles au Genre';
        $categoryName = 'Document budgétaire sensible au genre'; // Specific category for this controller
        ...
        // Retrieve all documents linked to the 'Document budgétaire sensible au genre' category
        ...
        // Auto-selected categories ('Document budgétaire sensible au genre' in this case) for the view
        ...
        $bladeRouteNames = [
            'show' => 'documents.budget_genre.show',
            'download' => 'documents.budget_genre.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // For JavaScript
                'search' => route('documents.budget_genre.search'),
                'show' => route('documents.budget_genre.show', ['slug' => 'PLACEHOLDER_SLUG']),
                'download' => route('documents.budget_genre.download', ['id' => 'PLACEHOLDER_ID'])
            ],
    ...
    /**
     * This method appears in your routes but not in your controller code.
     * It's commented out here but you might want to implement it if needed,
     * likely for showing all documents related to "Document budgétaire sensible au genre" without
     * the initial category filter constraints applied in `index()`.
     * Or, it could be redundant if `index()` covers the "all" use case.
     * I'm providing a basic structure if you decide to use it.
     */
    // public function all(): View
    // {
    //     $title = 'Tous les Documents Budgétaires Sensibles au Genre';
    //     $categoryName = 'Document budgétaire sensible au genre';
    ...
    {
        Log::debug('Search request received for "Document budgétaire sensible au genre" documents', [
        ...
        $categoryName = "Document budgétaire sensible au genre"; // Specific category for search
        $targetCategoryIds = Category::where('name', $categoryName)->pluck('id')->toArray();

        // Build the base query, filtered by the 'Document budgétaire sensible au genre' category
        ...
        // Note: These will be added to the 'Document budgétaire sensible au genre' filter already in place
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (show method - Budget Sensible Genre)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for display (show method - Budget Sensible Genre)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (download method - Budget Sensible Genre)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for download (download method - Budget Sensible Genre)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
