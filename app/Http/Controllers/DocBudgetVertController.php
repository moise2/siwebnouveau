...
class DocBudgetVertController extends Controller
{
    ...
    {
        $title = 'Documents Budget Vert';
        $categoryName = 'Budget vert'; // Specific category for this controller
        ...
        // Retrieve all documents linked to the 'Budget vert' category
        ...
        // Auto-selected categories ('Budget vert' in this case) for the view
        ...
        $bladeRouteNames = [
            'show' => 'documents.budget_vert.show',
            'download' => 'documents.budget_vert.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // For JavaScript
                'search' => route('documents.budget_vert.search'),
                'show' => route('documents.budget_vert.show', ['slug' => 'PLACEHOLDER_SLUG']),
                'download' => route('documents.budget_vert.download', ['id' => 'PLACEHOLDER_ID'])
            ],
     ...
     * likely for showing all documents related to "Budget vert" without
    ...
    // {
    //     $title = 'Tous les Documents Budget Vert';
    //     $categoryName = 'Budget vert';
    ...
    {
        Log::debug('Search request received for "Budget vert" documents', [
        ...
        $categoryName = "Budget vert"; // Specific category for search
        $targetCategoryIds = Category::where('name', $categoryName)->pluck('id')->toArray();

        // Build the base query, filtered by the 'Budget vert' category
        ...
        // Note: These will be added to the 'Budget vert' filter already in place
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (show method - Budget Vert)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for display (show method - Budget Vert)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (download method - Budget Vert)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for download (download method - Budget Vert)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
