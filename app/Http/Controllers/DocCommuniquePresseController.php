...
class DocCommuniquePresseController extends Controller
{
    ...
    /**
     * Displays the initial page for "Communiqués de Presse" documents.
     */
    public function index(): View
    {
        $title = 'Documents Communiqués de Presse';
        $categoryName = 'Communiqués de presse'; // Specific category for this controller
        ...
        // Auto-selected categories ('Communiqués de presse' in this case) for the view
        ...
        $bladeRouteNames = [
            'show' => 'documents.communique_presse.show',
            'download' => 'documents.communique_presse.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // For JavaScript
                'search' => route('documents.communique_presse.search'),
                'show' => route('documents.communique_presse.show', 'PLACEHOLDER_SLUG'), // Use a placeholder for dynamic routing
                'download' => route('documents.communique_presse.download', 'PLACEHOLDER_ID') // Use a placeholder for dynamic routing
            ],
    ...
    /**
     * Fetches ALL documents for the 'Communiqués de presse' category without pagination.
     * Implemented as JsonResponse for API-like access, consistent with previous refactorings.
     */
    ...
    /**
     * Handles AJAX search requests for "Communiqués de Presse" documents.
     * Delegates the heavy lifting to the DocumentSearchService.
     */
    public function search(Request $request): JsonResponse
    {
        Log::debug('Search request received for "Communiqués de presse" documents (delegated to service)', [
        ...
        $categoryName = "Communiqués de presse"; // Specific category for search
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (show method - Communiqués de presse)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for display (show method - Communiqués de presse)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (!$document->categories->contains('name', $categoryName)) {
            Log::warning("Attempt to download document from wrong category (download method - Communiqués de presse)", [
                'document_id' => $id,
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (download method - Communiqués de presse)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for download (download method - Communiqués de presse)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
