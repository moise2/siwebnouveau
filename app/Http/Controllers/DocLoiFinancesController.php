...
use Carbon\Carbon; // Still imported, potentially useful for date formatting in views if needed

class DocLoiFinancesController extends Controller
{
    ...
    /**
     * Displays the initial page for "Lois de finances" documents.
     */
    public function index(): View
    {
        $title = 'Documents Lois de Finances';
        $categoryName = 'Lois de finances'; // Specific category for this controller
        ...
        // Auto-selected categories ('Lois de finances' in this case) for the view
        ...
        $bladeRouteNames = [
            'show' => 'documents.loi_finances.show',
            'download' => 'documents.loi_finances.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // For JavaScript
                'search' => route('documents.loi_finances.search'),
                'show' => route('documents.loi_finances.show', 'PLACEHOLDER_SLUG'), // Use a placeholder for dynamic routing
                'download' => route('documents.loi_finances.download', 'PLACEHOLDER_ID') // Use a placeholder for dynamic routing
            ],
    ...
    /**
     * This method fetches ALL documents for the 'Lois de finances' category without pagination.
     * It's common to have an 'all' endpoint for API access or specific views.
     */
    public function all(): JsonResponse
    {
        $categoryName = "Lois de finances";
    ...
    /**
     * Handles AJAX search requests for "Lois de finances" documents.
     * Delegates the heavy lifting to the DocumentSearchService.
     */
    public function search(Request $request): JsonResponse
    {
        Log::debug('Search request received for "Lois de finances" documents (delegated to service)', [
        ...
        $categoryName = "Lois de finances"; // Specific category for search
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (show method - Lois de finances)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for display (show method - Lois de finances)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (!$document->categories->contains('name', $categoryName)) {
            Log::warning("Attempt to download document from wrong category (download method - Lois de finances)", [
                'document_id' => $id,
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (download method - Lois de finances)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for download (download method - Lois de finances)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
