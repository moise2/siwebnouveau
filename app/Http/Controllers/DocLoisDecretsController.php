...
class DocLoisDecretsController extends Controller
{
    ...
    {
        $title = 'Documents Décrets et lois/arrêtés';
        $categoryName = 'Décrets et lois/arrêtés'; // Specific category for this controller
        ...
        // Retrieve all documents linked to the 'Décrets et lois/arrêtés' category
        ...
        // Auto-selected categories ('Décrets et lois/arrêtés' in this case) for the view
        ...
        $bladeRouteNames = [
            'show' => 'documents.lois_decrets.show',
            'download' => 'documents.lois_decrets.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // For JavaScript
                'search' => route('documents.lois_decrets.search'),
                'show' => route('documents.lois_decrets.show', ['slug' => 'PLACEHOLDER_SLUG']),
                'download' => route('documents.lois_decrets.download', ['id' => 'PLACEHOLDER_ID'])
            ],
    ...
    /**
     * This method is present in your routes as 'all'.
     * You might want to implement it if needed, likely for showing all documents related
     * to "Décrets et lois/arrêtés" without the initial category filter constraints applied in `index()`.
     */
    public function all(): View
    {
        $title = 'Tous les Documents Décrets et lois/arrêtés';
        $categoryName = 'Décrets et lois/arrêtés';
    ...
    {
        Log::debug('Search request received for "Décrets et lois/arrêtés" documents', [
        ...
        $categoryName = "Décrets et lois/arrêtés"; // Specific category for search
        $targetCategoryIds = Category::where('name', $categoryName)->pluck('id')->toArray();

        // Build the base query, filtered by the 'Décrets et lois/arrêtés' category
        ...
        // Note: These will be added to the 'Décrets et lois/arrêtés' filter already in place
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (show method - Décrets et lois/arrêtés)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for display (show method - Décrets et lois/arrêtés)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (download method - Décrets et lois/arrêtés)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for download (download method - Décrets et lois/arrêtés)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
