...
class DocStrategieEndettementController extends Controller
{
    ...
    /**
     * Displays the initial page for "Stratégie d'endettement" documents.
     */
    public function index(): View
    {
        $title = 'Documents Stratégie d\'endettement';
        $categoryName = 'Stratégie d\'endettement'; // Specific category for this controller
        ...
        // Auto-selected categories ('Stratégie d'endettement' in this case) for the view
        ...
        $bladeRouteNames = [
            'show' => 'documents.strategie_endettement.show',
            'download' => 'documents.strategie_endettement.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // For JavaScript
                'search' => route('documents.strategie_endettement.search'),
                'show' => route('documents.strategie_endettement.show', 'PLACEHOLDER_SLUG'), // Placeholder for dynamic routing
                'download' => route('documents.strategie_endettement.download', 'PLACEHOLDER_ID') // Placeholder for dynamic routing
            ],
    ...
    /**
     * Handles AJAX search requests for "Stratégie d'endettement" documents.
     * Delegates the heavy lifting to the DocumentSearchService.
     */
    public function search(Request $request): JsonResponse
    {
        Log::debug('Search request received for "Stratégie d\'endettement" documents (delegated to service)', [
        ...
        $categoryName = "Stratégie d'endettement"; // Specific category for search
    ...
    /**
     * This method fetches ALL documents for the 'Stratégie d\'endettement' category without pagination.
     */
    public function all(): JsonResponse
    {
        $categoryName = "Stratégie d'endettement";
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (show method - Stratégie d'endettement)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for display (show method - Stratégie d'endettement)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (!$document->categories->contains('name', $categoryName)) {
            Log::warning("Attempt to download document from wrong category (download method - Stratégie d'endettement)", [
                'document_id' => $id,
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (download method - Stratégie d'endettement)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for download (download method - Stratégie d'endettement)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
