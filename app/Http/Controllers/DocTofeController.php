...
use Carbon\Carbon; // Still useful if you need to format dates in the controller

class DocTofeController extends Controller
{
    ...
    {
        $title = 'Documents TOFE (Le tableau des opérations financières de l\'État)';
        $categoryName = 'TOFE'; // Specific category for this controller
        ...
        // Auto-selected categories ('TOFE' in this case) for the view
        ...
        $bladeRouteNames = [
            'show' => 'documents.tofe.show',
            'download' => 'documents.tofe.download',
        ];
        ...
        return view('frontend.pages.page_documents', [
            ...
            'jsRoutes' => [ // For JavaScript
                'search' => route('documents.tofe.search'),
                'show' => route('documents.tofe.show', 'PLACEHOLDER_SLUG'), // Use a placeholder slug for JS
                'download' => route('documents.tofe.download', 'PLACEHOLDER_ID') // Use a placeholder ID for JS
            ],
    ...
    /**
     * Handles document search for 'TOFE' category, delegating to the search service.
     */
    public function search(Request $request): JsonResponse
    {
        Log::debug('Search request received for "TOFE" documents (delegated to service)', [
        ...
        $categoryName = "TOFE"; // Specific category for search
    ...
    /**
     * This method fetches ALL documents for the 'TOFE' category without pagination.
     */
    ...
    public function show(string $slug): Response
    {
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (show method - TOFE)", [
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for display (show method - TOFE)", [
                'document_id' => $document->id,
    ...
    public function download(int $id): BinaryFileResponse
    {
        ...
        if (!$document->categories->contains('name', $categoryName)) {
            Log::warning("Attempt to download document from wrong category (download method - TOFE)", [
                'document_id' => $id,
        ...
        if (is_string($filePathInDb)) {
            ...
            Log::error("File path in database is not a string for document (download method - TOFE)", [
                'document_id' => $document->id,
                'path_in_db' => $filePathInDb
            ]);
        ...
        if (!$realFilePath || !Storage::disk('public')->exists($realFilePath)) {
            Log::error("File not found on disk for download (download method - TOFE)", [
                'document_id' => $document->id,
                'extracted_path_used' => $realFilePath,
                'original_db_value' => $filePathInDb,
                'storage_exists_check' => Storage::disk('public')->exists($realFilePath) ? 'true' : 'false'
